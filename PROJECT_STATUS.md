...
- 最后更新：2026-09-09
- 当前阶段：Phase 2.3 PASS（SQLite 本地持久化完成）。
- 阶段门禁：停止等待用户确认，不进入 Phase 2.4 总验收。
- 已完成：Phase 0；Phase 1 全部；Phase 2.1 统一校验与添加；Phase 2.2 编辑/删除；Phase 2.3 SQLite migration、持久化 CRUD 与重启恢复。
- 核心规则：严格 HH:mm；课程 top/height 只由实际时间及 pxPerMinute 决定；07:00–22:00 轴保留真实空闲比例；重叠链由纯布局函数分配横向 lane；星期列始终为周一至周日。
- 验证 PASS：严格 typecheck、64 项 TypeScript 单元测试、28 项架构测试、7 项 Rust 数据库测试、9 组窗口/缩放矩阵中的 183 项 UI 测试（6 项按既有条件跳过）、clippy、oxlint、Prettier、npm run build、npm run verify。
- Desktop PASS：真实 Tauri WebView 与 AppData SQLite 完成四轮启动。添加后重启存在；编辑为“机械原理”后同一 ID、周四 09:00 的 120px 偏移和 90px 高度跨重启保留；删除后再次重启仍不存在。页面/控制台无错误。
...
- Phase 2.3 状态：数据库由 `app_local_data_dir()` 自动创建，schema `user_version=1`；fixture 只在开发模式显示且从未写入数据库。损坏行跳过并提示，不自动修改或删除。
- 尚未实现：Phase 2.4 总验收、PDF/教务导入、提醒、自启动、托盘、安装包。
- 已知非阻断项：Rust 的 linker_messages 创建库/对象输出警告仍存在，编译和运行正常。没有忽略失败测试。
- PDF：已有真实样本检查，15 条固定安排和 3 条非固定实践课程；原件不入 Git，实际钟点和学期起点仍需用户确认。
- 下一步：用户确认后才进入 Phase 2.4 关闭/重启恢复与 Phase 2 总验收，本轮停止。
- 验证详情：docs/phase-1-verification.md、docs/phase-2-verification.md。
- Git：验证通过后本地提交 feat: persist courses with sqlite；具体提交号见 git log -1，不推送远端。
