...
- v1.3.0 自动门禁：`npm run verify` PASS（UI 441 项，426 通过、15 跳过；TypeScript 单元测试 125 项），Rust 39 项测试、fmt、clippy PASS；`npm run tauri build` 已按本轮修复重新生成 v1.3.0 NSIS/MSI 与 updater signatures。剩余门禁仅为最新安装包的 Windows 10 人工验收；完成前不创建 tag、不发布 Release。
