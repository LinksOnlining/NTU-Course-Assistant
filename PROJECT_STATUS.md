...
- v1.3.0 发布门禁：`npm run verify` PASS（UI 450 项，435 通过、15 跳过；TypeScript 单元测试 128 项，架构测试 60 项），Rust 39 项测试、fmt、clippy PASS；生产 NSIS/MSI 与 updater signatures 已生成并通过安装态启动、单实例和 Windows 10 人工验收。v1.3.0 tag 与 GitHub Release 已发布。
- 已知限制：旧版无 `semesterId` 的课程继续按兼容策略处理；应用完全退出后不发送提醒；v1.3.0 尚未在 v1.2.1 安装态完成跨版本 updater E2E。
...
- 验证 PASS：严格 typecheck、112 项 TypeScript 单元测试、50 项架构测试、37 项 Rust 测试、441 个 UI 场景（426 通过、15 项按设备/私有样本条件跳过）、clippy、oxlint、Prettier、npm run build、npm run verify 与当前 HEAD 的 `npm run tauri build`。
...
- Phase 2.5–2.7 状态：`PeriodTime` 配置与 UI 解耦；作息要求第 1 节起连续编号、严格 HH:mm、节间不重叠且最多 30 节。test-only 作息可在设置中添加、修改和删除最后一节，并持久化到 SQLite；左轴按真实分钟显示节次与时间，时间轴自动按整点扩展，课间和午休不压缩。v1.3.1 的 `PERIOD_BASED` 课程通过 canonical resolver 动态读取当前确认作息，保存作息不批量改写兼容快照；固定钟点课程保持不变。
