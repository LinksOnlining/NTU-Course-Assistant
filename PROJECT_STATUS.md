...
- 最后更新：2026-09-22
- 当前阶段：**v1.3.0 Academic Hub Release Candidate**。CourseOccurrence/CourseOverride 统一读取模型、停课/调课/换教室/补课、学习事项、考试、今日学习中心、Widget NEXT/DEADLINES 模式与 ACTIVE/ARCHIVED 学期数据基础已完成；本轮 RC UI 收口新增紧凑学习中心标签与“课程 → 发生次数 → 操作”课表变化流程（课程去重、搜索/筛选、中文日期/状态、逐条操作和补课入口），自动门禁与 v1.3.0 生产构建 PASS，等待 Ethan 完成 Windows 10 安装态人工验收，尚未创建 v1.3.0 tag 或发布 Release。
...
- v1.3.0 自动门禁：`npm run verify` PASS（UI 450 项，435 通过、15 跳过；TypeScript 单元测试 128 项，架构测试 60 项），Rust 39 项测试、fmt、clippy PASS；`npm run tauri build` 已按本轮修复重新生成 v1.3.0 NSIS/MSI 与 updater signatures。剩余门禁仅为最新安装包的 Windows 10 人工验收；完成前不创建 tag、不发布 Release。
