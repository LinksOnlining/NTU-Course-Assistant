...
- 最后更新：2026-09-09
- 当前阶段：Phase 2.6 PASS（Phase 1、Phase 2 仍为 PASS；可配置作息与比例节次时间轴完成）。
- 阶段门禁：停止等待用户确认，不进入 Phase 3。
- 已完成：Phase 0；Phase 1 全部；Phase 2 全部；Phase 2.5 节次显示；Phase 2.6 用户可配置作息、SQLite 持久化与比例节次标记。
- 核心规则：严格 HH:mm；课程 top/height 只由实际时间及 pxPerMinute 决定；07:00–22:00 轴保留真实空闲比例；重叠链由纯布局函数分配横向 lane；星期列始终为周一至周日。
- 验证 PASS：严格 typecheck、73 项 TypeScript 单元测试、29 项架构测试、12 项 Rust 测试、252 个 UI 场景（246 通过、6 项按既有条件跳过）、clippy、oxlint、Prettier、npm run build、npm run verify。
- Desktop PASS：真实 Tauri WebView 从 schema 1 迁移到 schema 2；设置窗口保存 12 节自定义作息后关闭并重启，作息、真实分钟比例和 0 门用户课程均恢复，页面/控制台无错误，原课程时间未被改写。
...
- Phase 2.5/2.6 状态：`PeriodTime` 配置与 UI 解耦；作息要求第 1 节起连续编号、严格 HH:mm、节间不重叠且最多 30 节。test-only 作息可在设置中添加、修改和删除最后一节，并持久化到 SQLite；左轴按真实分钟显示节次与时间，时间轴自动按整点扩展，课间和午休不压缩。只有精确匹配作息的 Course 才显示节次，修改作息不会改写已有课程时间。
...
- 验证详情：docs/phase-1-verification.md、docs/phase-2-verification.md、docs/phase-2-5-verification.md、docs/phase-2-6-verification.md。
- Git：验证通过后本地提交 feat: add configurable period schedule；具体提交号见 git log -1，不推送远端。
