...
- 最后更新：2026-09-09
- 当前阶段：Phase 2.5 PASS（Phase 2 仍为 PASS；节次与实际时间轴显示完成）。
- 阶段门禁：停止等待用户确认，不进入 Phase 3。
- 已完成：Phase 0；Phase 1 全部；Phase 2 全部；Phase 2.5 test-only 节次配置、纯映射与连续时间轴标记。
- 核心规则：严格 HH:mm；课程 top/height 只由实际时间及 pxPerMinute 决定；07:00–22:00 轴保留真实空闲比例；重叠链由纯布局函数分配横向 lane；星期列始终为周一至周日。
- 验证 PASS：严格 typecheck、72 项 TypeScript 单元测试、29 项架构测试、9 项 Rust 数据库测试、9 组窗口/缩放矩阵中的 219 项 UI 测试（6 项按既有条件跳过）、clippy、oxlint、Prettier、npm run build、npm run verify。
...
- Phase 2.5 状态：`PeriodTime` 配置与 UI 解耦；1–11 节 test-only 作息通过纯函数严格校验并支持节次/时间双向精确映射。左轴按真实分钟显示节次与时间，课间和午休不压缩；只有 Course 已有节次时卡片才显示节次。
...
- 验证详情：docs/phase-1-verification.md、docs/phase-2-verification.md、docs/phase-2-5-verification.md。
- Git：验证通过后本地提交 feat: add period labels to timetable axis；具体提交号见 git log -1，不推送远端。
