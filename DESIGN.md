...
v1.3.1 以 `Course.startPeriod/endPeriod` 成对存在作为唯一时间语义判别：两个索引均存在时为 `PERIOD_BASED`，有效钟点只能由这两个索引与当前 `PeriodTime[]` 解析；两个索引均为空时为 `FIXED_TIME`，使用明确保存的 `startTime/endTime`。对于 `PERIOD_BASED`，SQLite 为兼容旧 schema 保留的 `startTime/endTime` 只是导入/写入快照，任何运行时解析、展示、提醒、Widget 或课程变化逻辑都不得将其作为回退；当前作息缺失或无法映射时不生成该 occurrence，也不删除原始课程。无需 schema migration 或批量改写课程数据。明确写入钟点的 RESCHEDULE/MAKEUP 保持固定单次时间；仅改变教室的 MODIFY 与 CANCEL 继续基于更新后的基础 occurrence。作息状态更新后，主课表/学习中心重新解析，提醒计划依赖新的 periods 重建，小组件从数据刷新中重新读取作息并解析；无学期兼容显示路径也使用当前作息解析有节次课程。
