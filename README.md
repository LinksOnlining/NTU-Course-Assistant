...
**Phase 3 PASS。** 真实 PDF 可提取并识别为 15 条固定安排和 3 条非固定实践候选；用户修正并最终确认后，应用使用单个 SQLite 事务原子写入课程。精确重复会跳过，时间冲突保留并提示，导入结果可在 Windows Tauri 重启后恢复。
...
- `npm run verify`：执行 typecheck、单元测试、架构测试、288 个 UI 场景、lint、格式检查和 build。
