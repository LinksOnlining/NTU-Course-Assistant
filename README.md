...
**Phase 2.7 PASS。** Phase 2 的本地课程 CRUD、SQLite 持久化和可配置作息保持通过；桌面时间轴将节次、开始和结束时间分层显示，并在日列中以轻量引导线对应课程的真实分钟位置。短课程保留名称、时间、教室和教师，通过自适应字号适配，不改变真实高度；当前仍使用明确标注的 test-only 作息，正式南通大学作息尚未配置。
