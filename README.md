...
**Phase 2 PASS。** 当前可在 Windows 11 独立运行，并能添加、编辑、删除和持久化用户课程。数据通过受限 Tauri command 写入 Windows 用户应用数据目录中的 SQLite；正常关闭和重新启动后，添加与编辑结果保留，删除结果也保持。损坏记录、未来 schema、数据库繁忙和写入失败均有明确边界，fixture 只在开发模式显示且不会写入数据库。
...
- `npm run verify`：执行 typecheck、单元测试、架构测试、207 个 UI 场景、lint、格式检查和 build。
...
详见 [真实 PDF 样本检查](docs/pdf-sample-review.md)、[Phase 1 验证](docs/phase-1-verification.md) 和 [Phase 2 验证](docs/phase-2-verification.md)。Phase 2 完成后停止等待确认，不进入 Phase 3。
