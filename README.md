...
**Phase 2.5 PASS。** Phase 2 的本地课程 CRUD 与 SQLite 持久化保持通过；连续时间轴现在附加显示 test-only 的第 1–11 节及各节开始/结束时间。节次标记按真实分钟定位，课间、午休和课程卡片几何不压缩。正式南通大学作息尚未配置。
...
- `npm run verify`：执行 typecheck、单元测试、架构测试、225 个 UI 场景、lint、格式检查和 build。
...
详见 [真实 PDF 样本检查](docs/pdf-sample-review.md)、[Phase 1 验证](docs/phase-1-verification.md)、[Phase 2 验证](docs/phase-2-verification.md) 和 [Phase 2.5 验证](docs/phase-2-5-verification.md)。Phase 2.5 完成后停止等待确认，不进入 Phase 3。
