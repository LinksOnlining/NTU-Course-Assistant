...
**Phase 5 PASS；Phase 4 已由用户取消。** 真实 PDF 可提取并识别为 15 条固定安排和 3 条非固定实践候选；用户修正并最终确认后，应用使用单个 SQLite 事务原子写入课程。运行中的 Windows 应用会在课程 due 时发送系统通知，并在应用重启或窗口恢复后重建提醒计划；完全退出时不提供后台提醒。
...
- `npm run verify`：执行 typecheck、单元测试、架构测试、297 个 UI 场景、lint、格式检查和 build。
