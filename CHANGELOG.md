...
## 1.3.1 — 2026-09-23（Release Candidate，尚未发布）

### Fixed

- 按节次安排的课程只以 `startPeriod/endPeriod` 为事实来源，当前钟点由统一解析器结合当前作息计算；兼容保留的导入钟点快照不参与运行时回退，无法映射时不显示旧时间。手动固定时间课程保持不变。
- 作息变更会更新周课表、今日中心、桌面小组件、课表变化管理和课程提醒；房间覆盖仍只改变地点，取消仍展示更新后的原安排，明确时间的调课/补课不跟随作息变化。
- 课程编辑在用户未改动派生钟点时保留原节次索引，避免只编辑名称或地点就把按节次课程意外转成固定时间。
...
- 修复 PDF 导入确认后可能长期停留在“保存中”的问题。
- 修复新增或修改节次时间后保存失败或长期保存中的问题。
- 修复数据库共享连接与锁生命周期可能导致的资源滞留。
- 修复保存失败后部分 UI 状态无法可靠恢复的问题。
- 改进重复提交防护和保存失败重试行为。

### Changed

- 数据库命令改为短生命周期连接策略，并将阻塞数据库操作移至后台执行路径。
- 受影响的保存流程使用明确的 typed save state，改进持久化错误传播和生命周期管理。
- 完成 Windows 10 安装态人工回归验证。

## 1.2.0 保存稳定性修复 — 2026-09-21

### Fixed

- 移除进程级共享 SQLite 连接和 Mutex，改为每个存储命令使用短生命周期连接，并在后台线程执行同步数据库读写，避免 PDF 导入后的运行时资源占用阻塞后续保存与文件选择器。
- 作息与小组件保存采用显式 validating/saving/success/error 状态，防止重复提交、永久“保存中”以及失败后无法重试。
- 新增新增/编辑作息的保存回读回归覆盖；不改变 SQLite schema（仍为 4）。

## 1.2.0 Release Candidate — 2026-09-20

### Added

- 应用内更新检查：启动后后台检查，设置“关于”页可手动检查；发现新版本时展示版本、发布日期、发布说明、稍后提醒和立即更新。
- 更新下载进度、失败重试与 GitHub Releases 回退入口；生产更新使用 Tauri 官方签名验证和 HTTPS GitHub Release endpoint。
- “回到现在”课表操作，以及 5 天/7 天视图偏好持久化。
- PDF 导入完成结果面板，显示成功导入、重复跳过、未导入和需要注意的实际统计。

### Changed

- v1.1.2 及更早版本需要手动安装 v1.2.0；后续版本可使用应用内更新。

### Removed

- v1.2.0 不再包含 Backup / Restore，SQLite schema 保持 4。

### Fixed

- 小组件在保存设置或作息后的读取与渲染不再占用 Tauri 命令运行时；窗口元数据短暂不可用时会保留渲染并重试绑定边界事件，避免白屏。PDF 文件选择器继续独立响应。
- 修复 PDF 文件选择器取消、返回空数组/`undefined` 或抛出异常后未释放导入操作状态的问题；取消后可立即继续保存小组件和作息设置，并允许再次打开文件选择器。
...
- 真实 Tauri 独立窗口完成 0→18 原子导入、立即显示、正常关闭重启恢复和第二次重复 18/写入 0；数据库始终保持 schema 2 和 12 条作息。验收前创建临时数据库副本并在结束后恢复原数据库，临时脚本、截图和样本副本均已清理。
