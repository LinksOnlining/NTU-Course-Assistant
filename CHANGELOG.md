...
- **Phase 2.7 PASS**：优化 Windows 桌面课程表的时间轴与课程对应关系。左侧以节次、开始和结束时间分层显示，日列新增按真实节次起点定位的轻量引导线；小时线降为辅助信息，课间、午休和连续分钟比例保持不变。
- 课程卡片改为宽松的桌面内边距、低饱和稳定配色和更清晰的信息层级。短课与重叠卡片根据高度及可用宽度自适应缩小字号，不再隐藏教室或教师，也不使用文本省略号；课程高度没有改变。
- 新增 UI 回归，覆盖节次块与精确课程的 top/height 对齐、作息修改后课程时间不变、引导线、舒适日列宽度及 45 分钟卡片完整字段。
