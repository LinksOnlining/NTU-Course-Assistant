...
- PDF 导入的按节次课程以节次索引作为时间事实来源；当前上课时间由当前作息解析。
- 修改作息后，已有按节次课程会立即更新，无需重新导入 PDF 或重启；课表、Today、Widget、Reminder 和课程变化使用一致的解析结果。
- 固定钟点课程不受作息变化影响；换教室覆盖保留教室但跟随新作息时间，明确调课/补课时间保持固定，停课状态保持不变。
- 编辑课程时保留未被用户修改的节次索引，避免派生时间覆盖节次语义。
